...
description: "Main-agent-only Browser Use. The main agent must perform browser work itself and must not delegate it to a subagent; subagents must not load this skill or use Browser Use. Use to open, navigate, inspect, test, click, type, fill, screenshot, or verify web pages and local HTTP targets (localhost, 127.0.0.1, ::1) inside IllusionAgent, including browser/web-UI automation, rendered-page scraping, frontend checks, and visible page-state reading."
---
...
If the user explicitly asks for IllusionAgent's in-app browser:
...
1. Start every browser `js` call with the bootstrap, then assign the selected backend to a local `browser` binding. If the user explicitly asks for IllusionAgent's in-app browser, use `const browser = await agent.browsers.get("iab")`. If they explicitly ask for Chrome, use `await agent.browsers.get("extension")` only when the runtime advertises it. For an unspecified target URL use `await agent.browsers.getForUrl(url)`; with no URL/backend preference use `await agent.browsers.getDefault()`.
...
8. Browser tabs persist for the lifetime of the current IllusionAgent process unless you explicitly call `tab.close()` or
...
- `tab.playwright.waitForTimeout(timeoutMs)` — Codex-compatible fixed wait for the rare case where no concrete
  ...
  `tab.waitForTimeout(...)`; that root-level API does not exist in Codex or IllusionAgent. Prefer a targeted wait or fresh `domSnapshot()`
  over routine sleeps.
